...
static const char_t* bucketTypeToString(ECO_BUCKET_SORT_TYPE typeId) {
    switch (typeId) {
    case ECO_BUCKET_SORT_INT32:
        return "int32";
    case ECO_BUCKET_SORT_LONG:
        return "long";
    case ECO_BUCKET_SORT_FLOAT:
        return "float";
    case ECO_BUCKET_SORT_DOUBLE:
        return "double";
    case ECO_BUCKET_SORT_LONG_DOUBLE:
        return "long double";
    default:
        return "unknown";
    }
...
static void printArrayPreview(const void* pArray, int32_t size, ECO_BUCKET_SORT_TYPE typeId) {
...
int16_t ECOCALLMETHOD CEcoLab1Sink_OnBucketSortDistribute(/* in */ struct IEcoLab1Events* me,
    ...
    pCMe->m_iStep++;
    printf("Step %u [%s]: distribute element %d to bucket %d\n", pCMe->m_iStep, bucketTypeToString(typeId), elementIndex, bucketIndex);

    return 0;
}

int16_t ECOCALLMETHOD CEcoLab1Sink_OnBucketSortBucketSort(/* in */ struct IEcoLab1Events* me,
                                                          /* in */ int32_t bucketIndex,
                                                          /* in */ ECO_BUCKET_SORT_TYPE typeId) {
    if (me == 0) {
        return -1;
    ...
    printf("[Bucket:%s] sorting bucket %d\n", bucketTypeToString(typeId), bucketIndex);
...
int16_t ECOCALLMETHOD CEcoLab1Sink_OnBucketSortCompleted(/* in */ struct IEcoLab1Events* me,
    ...
    printf("[Bucket:%s] completed. Total elements: %d\n", bucketTypeToString(typeId), size);
...
IEcoLab1VTblEvents g_x2D2E3B9214F248A6A09ECB494B59C795VTblEvents = {
    ...
    CEcoLab1Sink_OnBucketSortDistribute,
    CEcoLab1Sink_OnBucketSortBucketSort,
